...
export default function ResultsStripImagePage() {
    ...
    <div className="min-h-dvh bg-[#070b16] pb-24 pt-24 text-white">
      <main className="mx-auto flex w-full max-w-6xl flex-col gap-10 px-4 sm:px-6 lg:px-8">
        <header className="space-y-3">
          <p className="text-sm uppercase tracking-widest text-pink-300/80">strip test β</p>
          <h1 className="text-3xl font-bold md:text-4xl">画像(TypeImage)のみ非表示</h1>
          <p className="text-sm text-white/70">
            ヘッダー構造は残しつつ `TypeImage` 部分だけ display:none にしています。画像が原因か切り分け可能です。
          </p>
          <div className="flex flex-wrap gap-3 text-sm text-pink-200/80">
            ...
            <Link href="/test/results-strip-main" className="underline underline-offset-4">メイン本文OFF</Link>
        ...
        <section
          ref={containerRef}
          className="results-strip-image relative overflow-x-auto rounded-3xl border border-white/20 bg-white/5 p-4 shadow-lg backdrop-blur"
        >
          {mounted ? (
      ...
      <style jsx global>{`
        .results-strip-image .p-8.text-white.flex.justify-center .w-64 {
          display: none !important;
        }
        .results-strip-image .p-8.text-white.flex.justify-center .w-full > :last-child {
          display: none !important;
        }
      `}</style>
    </div>

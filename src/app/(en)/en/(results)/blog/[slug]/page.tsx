...
export async function generateStaticParams() {
  const slugs = getAllEnPostSlugs();
  return slugs.map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const post = getEnPostBySlug(params.slug);

  if (!post) {
    return {
      title: 'Article Not Found | Night Personality Test',
    };
  }
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        'ja': `https://nightpersonality.com/blog/${params.slug}`,
        'en': `https://nightpersonality.com/en/blog/${params.slug}`,
      },
    },
  };
}
...
export default function EnBlogPostPage({ params }: Props) {
  const post = getEnPostBySlug(params.slug);
  const { prev, next } = getAdjacentEnPosts(params.slug);
